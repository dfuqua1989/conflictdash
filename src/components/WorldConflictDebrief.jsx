  ...
  return <div>
    ...
    {tab==="timeline"&&<div>
      <Hero t={t} style={{borderLeft:"4px solid #8b5cf6"}}>
        ...
        <div style={{fontSize:12,color:"rgba(255,255,255,.65)",lineHeight:1.6}}>From Operation Epic Fury (Feb 28) through the Islamabad MoU and the 42-day countdown to Aug 16.</div>
      </Hero>
...
function NuclearSection({t}){
  ...
  const VECTORS=[
    ...
    {flag:"🇮🇷",name:"Iran — Nuclear Clock",status:"42 DAYS",color:"#8b5cf6",text:"Islamabad MoU 60-day window closes ~Aug 16. IAEA inspector access disputed. Enrichment red lines unresolved — Pezeshkian: 'we will never back down from the right to enrich uranium.'"},
...
function CyberSection({t}){
  ...
  return <div>
    <div onTouchStart={e=>e.stopPropagation()} onTouchMove={e=>e.stopPropagation()} onTouchEnd={e=>e.stopPropagation()} style={{display:"flex",gap:6,overflowX:"auto",paddingBottom:8,marginBottom:12,WebkitOverflowScrolling:"touch"}}>{TABS.map(tb=><button key={tb.id} className="pill-tab" onClick={()=>setTab(tb.id)} style={{flexShrink:0,padding:"6px 12px",borderRadius:20,border:`1px solid ${tab===tb.id?"#5b8ec8":t.border}`,background:tab===tb.id?"#5b8ec8":"none",color:tab===tb.id?"#fff":t.sub,fontSize:12,fontWeight:tab===tb.id?700:400,cursor:"pointer",fontFamily:FONT,whiteSpace:"nowrap"}}>{tb.label}</button>)}</div>
