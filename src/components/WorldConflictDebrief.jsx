...
function ClientDate({iso,style}){const[s,setS]=useState("");useEffect(()=>{setS(new Date(iso).toLocaleDateString([],{month:"short",day:"numeric"}));},[iso]);return <span style={style} suppressHydrationWarning>{s}</span>;}
...
const CONFLICTS=[
  ...
  {id:"caribbean",name:"Caribbean / Cuba Blockade",region:"Latin America",status:"BLOCKADE",statusColor:"#f97316",icon:"🇨🇺",deaths:"Infant mortality 9.9/1,000",displaced:"Medicine supply at 30% of normal",summary:"US oil blockade on Cuba since Jan 2026 — the first effective blockade since the Missile Crisis. Food production down 60%, rolling blackouts, and a deepening humanitarian crisis; maritime enforcement overlaps Operation Southern Spear’s strike zone. Opens the Venezuela / Caribbean section → Cuba Blockade tab.",warDay:null},
  {id:"venezuela",name:"Venezuela",region:"Latin America",status:"US INTERVENTION",statusColor:"#dc2626",icon:"🇻🇪",deaths:"221+ killed in boat strikes",displaced:"Cuba humanitarian crisis deepening",summary:"US captured Maduro Jan 3, 2026 in Operation Absolute Resolve. Operation Southern Spear boat strikes ongoing — 221+ killed across 64+ strikes as of Jun 21. Concurrent US oil blockade on Cuba since Jan 2026 — first effective blockade since the Missile Crisis — has driven a deepening humanitarian crisis: infant mortality up to 9.9/1,000, food production down 60%, medicine supply at 30% of normal.",warDay:null},
];
...
const LOSSES={ukraine:[
  {id:1,category:"Personnel",total:"1,416,280",todayChange:"+1,460",color:"#ef4444",icon:"☠️",description:"Killed & wounded (est.)"},
  {id:2,category:"Tanks",total:"12,108",todayChange:"+1",color:"#f97316",icon:"🪖",description:"Main battle tanks"},
  {id:3,category:"Armoured Vehs",total:"24,912",todayChange:"+6",color:"#eab308",icon:"🚧",description:"APCs, IFVs, MRAPs"},
  {id:4,category:"Artillery",total:"45,680",todayChange:"+52",color:"#84cc16",icon:"💥",description:"Guns, howitzers, mortars"},
  {id:5,category:"MLRS",total:"1,923",todayChange:"+1",color:"#22c55e",icon:"🚀",description:"Multiple launch rockets"},
  {id:6,category:"Air Defence",total:"1,481",todayChange:"+2",color:"#06b6d4",icon:"🛡️",description:"SAM & SHORAD systems"},
  {id:7,category:"UAVs",total:"400,631",todayChange:"+1,868",color:"#5b8ec8",icon:"🛸",description:"Operational UAVs"},
  ...
  {id:12,category:"Vehicles",total:"118,249",todayChange:"+339",color:"#a78bfa",icon:"🚛",description:"Trucks & logistics"},
  {id:13,category:"Ground Robots",total:"1,865",todayChange:"+8",color:"#f43f5e",icon:"🤖",description:"UGV platforms"},
...
const REPORT_NOW=new Date("2026-07-10T12:00:00Z");
...
<Hero t={t}><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:10}}><span style={{width:8,height:8,borderRadius:"50%",background:"#ef4444",display:"inline-block",animation:"blink 1.4s ease-in-out infinite",flexShrink:0}}/><span style={{fontSize:11,color:"rgba(255,255,255,.45)",letterSpacing:".08em",textTransform:"uppercase"}}>War Day {UA_WAR_DAY} · Live</span><span style={{marginLeft:"auto",fontSize:11,color:"rgba(255,255,255,.35)"}}>{REPORT_NOW.toLocaleDateString("en-GB",{day:"numeric",month:"long",year:"numeric"})}</span></div><div style={{fontSize:11,color:"rgba(255,255,255,.4)",textTransform:"uppercase",letterSpacing:".05em"}}>🇷🇺 Total Russian Personnel Losses</div><div style={{fontSize:42,fontWeight:800,letterSpacing:-2,lineHeight:1,fontVariantNumeric:"tabular-nums"}}>{LOSSES.ukraine[0].total}</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",marginTop:14,paddingTop:12,borderTop:"1px solid rgba(255,255,255,.08)"}}><div><div style={{fontSize:11,color:"rgba(255,255,255,.4)"}}>Today</div><div style={{fontSize:22,fontWeight:800,color:"#ff9500",fontVariantNumeric:"tabular-nums"}}>+1,190</div></div><div><div style={{fontSize:11,color:"rgba(255,255,255,.4)"}}>7-Day Avg</div><div style={{fontSize:20,fontWeight:700,color:"#fbbf24",fontVariantNumeric:"tabular-nums"}}>~1,231</div></div><div><div style={{fontSize:11,color:"rgba(255,255,255,.4)"}}>Since Feb 22</div><div style={{fontSize:15,fontWeight:700,color:"rgba(255,255,255,.6)",marginTop:3}}>Day {UA_WAR_DAY}</div></div></div></Hero><Grid2 t={t} items={[{icon:"🛸",label:"UAVs Destroyed Today",val:LOSSES.ukraine[6].todayChange,sub:"cumulative "+LOSSES.ukraine[6].total,color:"#5b8ec8"},{icon:"☠️",label:"Personnel Today",val:LOSSES.ukraine[0].todayChange,sub:"cumulative "+LOSSES.ukraine[0].total,color:"#8b5cf6"},{icon:"🪖",label:"Tanks Today",val:LOSSES.ukraine[1].todayChange,sub:"cumulative "+LOSSES.ukraine[1].total,color:"#f97316"},{icon:"💥",label:"Artillery Today",val:LOSSES.ukraine[3].todayChange,sub:"cumulative "+LOSSES.ukraine[3].total,color:"#ef4444"}]}/><ST t={t}>📋 This Week in the War</ST><Card t={t}>{WEEK_UPDATES.map((u,i)=><Row key={i} t={t} last={i===WEEK_UPDATES.length-1}><div style={{fontSize:12.5,color:t.text,lineHeight:1.55}}>{u}</div></Row>)}</Card><div style={{background:t.isDark?"linear-gradient(135deg,#091321,#0d1f38)":"linear-gradient(135deg,#eef3fc,#e6edf9)",border:"1px solid rgba(59,130,246,0.25)",borderLeft:"4px solid #5b8ec8",borderRadius:14,padding:"14px 16px",marginBottom:16}}>
  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
    ...
    <ClientDate iso={UKRAINE_BRIEFING.generatedAt} style={{marginLeft:"auto",fontSize:10,color:t.sub}} />
  </div>
  ...
  return <div>
    ...
    {tab==="timeline"&&<div>
      <Hero t={t} style={{borderLeft:"4px solid #8b5cf6"}}>
        ...
        <div style={{fontSize:12,color:"rgba(255,255,255,.65)",lineHeight:1.6}}>From Operation Epic Fury (Feb 28) through the Islamabad MoU and the 37-day countdown to Aug 16.</div>
      </Hero>
    ...
    {tab==="gaps"&&<div>
      <Hero t={t} style={{borderLeft:"4px solid #ef4444"}}>
        <div style={{fontSize:11,color:"rgba(255,255,255,.4)",textTransform:"uppercase",letterSpacing:".08em",marginBottom:8}}>⚠️ Unresolved Gaps — 37 Days Remain</div>
...
function SudanSection({t,initialTab}){
  const[tab,setTab]=useState(initialTab??"conflict");useEffect(()=>{if(initialTab)setTab(initialTab);},[initialTab]);
  ...
  return <div>
    ...
    {tab==="conflict"&&<div>
      ...
      <Card t={t} style={{borderLeft:"4px solid #ef4444"}}><div style={{padding:"12px 14px"}}>
        {["RSF has surrounded El-Obeid from north, south, and west — 500K civilians at risk. ACLED: SAF ground offensive expected August 2026 to relieve siege","El-Obeid controls the highway crossroads linking Khartoum, Darfur, Kordofan, and southern Sudan — its fall would be decisive","El Fasher precedent: RSF overran North Darfur capital Oct 2025, committing extrajudicial killings immediately — UN confirmed genocide risk","RSF armed with Shahed drones and Turkish Bayraktar Akinci — SAF shot down one Akinci over Nyala Airport","38 NGOs including Amnesty and HRW demanded UNHRC emergency session Jun 26. 7 European nations issued joint warning."].map((pt,i)=><div key={i} style={{display:"flex",gap:8,marginBottom:6,fontSize:12,color:t.sub,lineHeight:1.6}}><span style={{color:"#ef4444",flexShrink:0}}>▸</span>{pt}</div>)}
      </div></Card>
...
function NuclearSection({t,initialTab}){
  ...
  const VECTORS=[
    ...
    {flag:"🇮🇷",name:"Iran — Nuclear Clock",status:"37 DAYS",color:"#8b5cf6",text:"Islamabad MoU 60-day window closes ~Aug 16. IAEA inspector access disputed. Enrichment red lines unresolved — Pezeshkian: 'we will never back down from the right to enrich uranium.'"},
...
function VenezuelaSection({t,initialTab}){
  ...
  const TABS=[{id:"conflict",label:"⚔️ Conflict"},{id:"transition",label:"🗳️ Transition"},{id:"spear",label:"🚤 Southern Spear"},{id:"cuba",label:"🇨🇺 Cuba Blockade"}];
...
const SECTION_TAB_LISTS={
  ...
  venezuela:[{id:"conflict",label:"⚔️ Conflict"},{id:"transition",label:"🗳️ Transition"},{id:"spear",label:"🚤 Southern Spear"},{id:"cuba",label:"🇨🇺 Cuba Blockade"}],
  ...
  sudan:[{id:"conflict",label:"⚔️ Conflict"},{id:"humanitarian",label:"🆘 Humanitarian"},{id:"southsudan",label:"🇸🇸 South Sudan"}],
  gaza:[{id:"conflict",label:"⚔️ Conflict"},{id:"humanitarian",label:"🆘 Humanitarian"},{id:"governance",label:"🌐 Governance"}],
  belarus:[{id:"overview",label:"🇧🇾 Overview"},{id:"basing",label:"🪖 Mobilization & Basing"},{id:"nuclear",label:"☢️ Nuclear Hosting"},{id:"homefront",label:"🏠 Home Front"},{id:"timeline",label:"📜 Timeline"}],
