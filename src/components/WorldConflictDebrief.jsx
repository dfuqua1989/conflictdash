...
const NEWS=[
  ...
  {id:"b_patriot_license_jul9",conflictId:"ukraine",severity:"critical",icon:"🔴",headline:"Trump Grants Ukraine a License to Manufacture Patriot Missiles — 'Make Them Yourself'",conflict:"Ukraine",conflictColor:"#5b8ec8",publishedAt:"Jul 9, 2026",bullets:["At Ankara, Trump announced he's licensing Ukraine to produce Patriot air-defense missiles itself; Zelensky said the licence is agreed 'at the political level' with PAC-3 supplies due 'in the coming days,' though officials caution real production is 12-24 months out","Poland separately agreed to a European PAC-3 maintenance center and is transferring 5 Patriot missiles directly; Ukraine is also pushing its own 'Freyja' interceptor. The urgency: CSIS says Ukraine intercepted only 14 of 54 Russian ballistic missiles in June","Jul 20 update: Patriot manufacturers have begun evaluating potential factory construction inside Ukraine — the first concrete step past the political-level agreement, though still short of a production timeline"],sources:["WORLD Radio","Kyiv Independent","Ukrinform"],impact:"The single most consequential materiel decision of the war to date, if it becomes real production — the binding constraint on Ukraine's cities has been PAC-3 supply",impactColor:"#dc2626"},
];

const LOSSES={ukraine:[
  {id:1,category:"Personnel",total:"1,434,690",todayChange:"+1,460",color:"#ef4444",icon:"☠️",description:"Killed & wounded (est.)"},
  {id:2,category:"Tanks",total:"12,191",todayChange:"+28",color:"#f97316",icon:"🪖",description:"Main battle tanks"},
  {id:3,category:"Armoured Vehs",total:"24,994",todayChange:"+12",color:"#eab308",icon:"🚧",description:"APCs, IFVs, MRAPs"},
  {id:4,category:"Artillery",total:"46,570",todayChange:"+78",color:"#84cc16",icon:"💥",description:"Guns, howitzers, mortars"},
  {id:5,category:"MLRS",total:"1,961",todayChange:"+3",color:"#22c55e",icon:"🚀",description:"Multiple launch rockets"},
  {id:6,category:"Air Defence",total:"1,515",todayChange:"+2",color:"#06b6d4",icon:"🛡️",description:"SAM & SHORAD systems"},
  {id:7,category:"UAVs",total:"423,588",todayChange:"+1,688",color:"#5b8ec8",icon:"🛸",description:"Operational UAVs"},
  ...
  {id:10,category:"Cruise Missiles",total:"4,936",todayChange:"+3",color:"#14b8a6",icon:"🎯",description:"Destroyed in flight"},
  {id:11,category:"Ships/Boats",total:"34",todayChange:"—",color:"#64748b",icon:"⛵",description:"Naval vessels"},
  {id:12,category:"Vehicles",total:"124,243",todayChange:"+541",color:"#a78bfa",icon:"🚛",description:"Trucks & logistics"},
  {id:13,category:"Ground Robots",total:"1,994",todayChange:"+13",color:"#f43f5e",icon:"🤖",description:"UGV platforms"},
  {id:14,category:"Special Equip",total:"4,453",todayChange:"+3",color:"#fb923c",icon:"🔧",description:"Engineering & EW"},
],gaza:[
...
const REPORT_NOW=new Date("2026-07-24T01:35:00Z");
...
function TodayView({t}){const critical=NEWS.filter(n=>n.severity==="critical");const watch=NEWS.filter(n=>n.severity==="watch"||n.severity==="major");const criticalShown=critical.slice(0,5);const watchShown=watch.slice(0,7-criticalShown.length);const shown=[...criticalShown,...watchShown];const shownCritical=shown.filter(n=>n.severity==="critical").length;const shownMajor=shown.filter(n=>n.severity==="major").length;const shownWatch=shown.filter(n=>n.severity==="watch").length;return <div style={{padding:"16px 16px 0",animation:"fadeIn .25s ease-out",position:"relative"}}><SJMark size={30} style={{top:2,right:6,transform:"rotate(-6deg)"}}/><BriefingPanel t={t}/>
    <div style={{background:t.isDark?"linear-gradient(135deg,#0d1622,#101c2b)":"#eef1f6",border:`1px solid ${t.border}`,borderLeft:"4px solid #dc2626",borderRadius:12,padding:"12px 14px",marginBottom:14}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:8}}>
        ...
        <div style={{fontSize:9,color:t.sub,opacity:.7,fontVariantNumeric:"tabular-nums"}}>WCD-{getWarDay()}</div>
      </div>
