...
const LOSSES={ukraine:[
  {id:1,category:"Personnel",total:"1,423,280",todayChange:"+1,470",color:"#ef4444",icon:"☠️",description:"Killed & wounded (est.)"},
  {id:2,category:"Tanks",total:"12,141",todayChange:"+10",color:"#f97316",icon:"🪖",description:"Main battle tanks"},
  {id:3,category:"Armoured Vehs",total:"24,938",todayChange:"+6",color:"#eab308",icon:"🚧",description:"APCs, IFVs, MRAPs"},
  {id:4,category:"Artillery",total:"45,953",todayChange:"+42",color:"#84cc16",icon:"💥",description:"Guns, howitzers, mortars"},
  {id:5,category:"MLRS",total:"1,936",todayChange:"+5",color:"#22c55e",icon:"🚀",description:"Multiple launch rockets"},
  {id:6,category:"Air Defence",total:"1,492",todayChange:"+1",color:"#06b6d4",icon:"🛡️",description:"SAM & SHORAD systems"},
  {id:7,category:"UAVs",total:"409,204",todayChange:"+2,003",color:"#5b8ec8",icon:"🛸",description:"Operational UAVs"},
  ...
  {id:10,category:"Cruise Missiles",total:"4,906",todayChange:"+7",color:"#14b8a6",icon:"🎯",description:"Destroyed in flight"},
  {id:11,category:"Ships/Boats",total:"34",todayChange:"+1",color:"#64748b",icon:"⛵",description:"Naval vessels"},
  {id:12,category:"Vehicles",total:"120,307",todayChange:"+445",color:"#a78bfa",icon:"🚛",description:"Trucks & logistics"},
  {id:13,category:"Ground Robots",total:"1,907",todayChange:"+2",color:"#f43f5e",icon:"🤖",description:"UGV platforms"},
  {id:14,category:"Special Equip",total:"4,420",todayChange:"+4",color:"#fb923c",icon:"🔧",description:"Engineering & EW"},
],gaza:[
...
const REPORT_NOW=new Date("2026-07-16T12:00:00Z");
  ...
  return <div>
    ...
    {tab==="timeline"&&<div>
      <Hero t={t} style={{borderLeft:"4px solid #8b5cf6"}}>
        ...
        <div style={{fontSize:12,color:"rgba(255,255,255,.65)",lineHeight:1.6}}>From Operation Epic Fury (Feb 28) through the Islamabad MoU and the 31-day countdown to Aug 16.</div>
      </Hero>
    ...
    {tab==="gaps"&&<div>
      <Hero t={t} style={{borderLeft:"4px solid #ef4444"}}>
        <div style={{fontSize:11,color:"rgba(255,255,255,.4)",textTransform:"uppercase",letterSpacing:".08em",marginBottom:8}}>⚠️ Unresolved Gaps — 31 Days Remain</div>
...
function NuclearSection({t,initialTab}){
  ...
  const TABS=[{id:"vectors",label:"⚠️ Active Vectors"},{id:"arsenals",label:"🌐 Global Arsenals"},{id:"delivery",label:"🚀 Delivery Systems"},{id:"treaties",label:"📜 Treaties & Doctrine"},{id:"cables",label:"⚓ Undersea Cables"},{id:"ew",label:"🛰️ Electronic Warfare"},{id:"cyber",label:"🌪️ Cyber Intrusions"},{id:"greyzone",label:"🎭 Grey-Zone"}];
  ...
  const VECTORS=[
    ...
    {flag:"🇮🇷",name:"Iran — Nuclear Clock",status:"31 DAYS",color:"#8b5cf6",text:"Islamabad MoU 60-day window closes ~Aug 16. IAEA inspector access disputed. Enrichment red lines unresolved — Pezeshkian: 'we will never back down from the right to enrich uranium.'"},
...
function USMilSection({t,initialTab}){
  ...
  return <div>
    ...
    {tab==="rankings"&&<div>
      ...
      <Card t={t}><div style={{padding:"10px 14px"}}>
        {GLOBAL_MIL_RANKINGS.map((c,i)=><div key={i} style={{marginBottom:10}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:3,fontSize:12}}><span style={{color:t.text,fontWeight:600}}>{c.flag} {c.name}</span><span style={{color:c.rank===1?"#8b5cf6":t.sub,fontWeight:700}}>{c.stealth?c.stealth.toLocaleString():"0"}</span></div>
          <div style={{height:6,background:t.isDark?"rgba(255,255,255,.08)":"rgba(0,0,0,.08)",borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:`${Math.max(c.stealth?3:0,c.stealth/RANK_MAX.stealth*100)}%`,background:c.rank===1?"#8b5cf6":"#a78bfa",borderRadius:3,transformOrigin:"left",animation:`barGrow .6s cubic-bezier(.22,1,.36,1) ${i*0.04}s both`}}/></div>
        </div>)}
        <div style={{fontSize:10.5,color:t.sub,fontStyle:"italic",marginTop:4}}>In-service F-35/F-22/J-20/J-35/Su-57 only — programs still in development (India's AMCA, France's SCAF/FCAS, Türkiye's Kaan) show 0 until aircraft actually reach squadrons. See 5th-Gen Fighters tab for per-aircraft detail.</div>
      </div></Card>
      <ST t={t} color="#22c55e">🛸 Unmanned Aircraft (All Classes)</ST>
      <Card t={t}><div style={{padding:"10px 14px"}}>
        {GLOBAL_MIL_RANKINGS.map((c,i)=><div key={i} style={{marginBottom:10}}>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:3,fontSize:12}}><span style={{color:t.text,fontWeight:600}}>{c.flag} {c.name}</span><span style={{color:c.rank===1?"#22c55e":t.sub,fontWeight:700}}>{c.drones.toLocaleString()}</span></div>
          <div style={{height:6,background:t.isDark?"rgba(255,255,255,.08)":"rgba(0,0,0,.08)",borderRadius:3,overflow:"hidden"}}><div style={{height:"100%",width:`${Math.max(3,c.drones/RANK_MAX.drones*100)}%`,background:c.rank===1?"#22c55e":"#4ade80",borderRadius:3,transformOrigin:"left",animation:`barGrow .6s cubic-bezier(.22,1,.36,1) ${i*0.04}s both`}}/></div>
        </div>)}
        <div style={{fontSize:10.5,color:t.sub,fontStyle:"italic",marginTop:4}}>Small/tactical/strategic UAVs combined — dominated by low-cost tactical drones, not high-end strike platforms. Türkiye's fleet size varies widely by source (1,400-3,000+); shown figure is a rough midpoint. Cross-reference: Drone War section for Ukraine/Russia's much larger wartime-consumption fleets, off this global peacetime comparison.</div>
      </div></Card>
