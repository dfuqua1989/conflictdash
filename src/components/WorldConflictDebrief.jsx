...
const LOSSES={ukraine:[
  {id:1,category:"Personnel",total:"1,409,630",todayChange:"+1,290",color:"#ef4444",icon:"☠️",description:"Killed & wounded (est.)"},
  {id:2,category:"Tanks",total:"12,081",todayChange:"+7",color:"#f97316",icon:"🪖",description:"Main battle tanks"},
  {id:3,category:"Armoured Vehs",total:"24,878",todayChange:"+9",color:"#eab308",icon:"🚧",description:"APCs, IFVs, MRAPs"},
  {id:4,category:"Artillery",total:"45,388",todayChange:"+63",color:"#84cc16",icon:"💥",description:"Guns, howitzers, mortars"},
  {id:5,category:"MLRS",total:"1,916",todayChange:"+3",color:"#22c55e",icon:"🚀",description:"Multiple launch rockets"},
  {id:6,category:"Air Defence",total:"1,470",todayChange:"+1",color:"#06b6d4",icon:"🛡️",description:"SAM & SHORAD systems"},
  {id:7,category:"UAVs",total:"390,738",todayChange:"+1,628",color:"#5b8ec8",icon:"🛸",description:"Operational UAVs"},
  ...
  {id:10,category:"Cruise Missiles",total:"4,847",todayChange:"—",color:"#14b8a6",icon:"🎯",description:"Destroyed in flight"},
  {id:11,category:"Ships/Boats",total:"33",todayChange:"—",color:"#64748b",icon:"⛵",description:"Naval vessels"},
  {id:12,category:"Vehicles",total:"116,106",todayChange:"+462",color:"#a78bfa",icon:"🚛",description:"Trucks & logistics"},
  {id:13,category:"Ground Robots",total:"1,821",todayChange:"+6",color:"#f43f5e",icon:"🤖",description:"UGV platforms"},
  {id:14,category:"Special Equip",total:"4,385",todayChange:"—",color:"#fb923c",icon:"🔧",description:"Engineering & EW"},
],gaza:[
...
const REPORT_NOW=new Date("2026-07-06T12:00:00Z");
...
function TheatersOverview({t,conflicts,onSelectConflict}){const STATUS_ORDER={"ESCALATING":0,"NEW CIVIL WAR":0,"US INTERVENTION":0,"CO-BELLIGERENT":1,"FLASHPOINT":1,"BLOCKADE":1,"Active":2,"MoU":3,"ELEVATED":4,"Fragile Ceasefire":5};const sorted=[...conflicts].sort((a,b)=>(STATUS_ORDER[a.status]??9)-(STATUS_ORDER[b.status]??9));const SECTION_MAP={"south-china-sea":"scs","taiwan":"scs","southsudan":"sudan","caribbean":"venezuela"};return <div><div style={{fontSize:10,fontWeight:700,color:t.sub,textTransform:"uppercase",letterSpacing:".1em",marginBottom:14}}>🌍 {conflicts.length} Active Theaters — Tap to Deep Dive</div><div style={{display:t.isLandscape?"grid":"block",gridTemplateColumns:t.isLandscape?"1fr 1fr":undefined,gap:t.isLandscape?12:0}}>{sorted.map((c,ci)=>{const sectionId=SECTION_MAP[c.id]??c.id;return <button key={c.id} className="theater-card" onPointerDown={e=>{const r=e.currentTarget.getBoundingClientRect();e.currentTarget.style.setProperty("--px",`${((e.clientX-r.left)/r.width*100)}%`);e.currentTarget.style.setProperty("--py",`${((e.clientY-r.top)/r.height*100)}%`);}} onClick={()=>onSelectConflict(sectionId)} style={{display:"block",width:"100%",background:t.card,borderTop:`1px solid ${t.border}`,borderRight:`1px solid ${t.border}`,borderBottom:`1px solid ${t.border}`,borderLeft:`4px solid ${c.statusColor}`,borderRadius:14,padding:"16px 18px",marginBottom:12,cursor:"pointer",fontFamily:FONT,textAlign:"left",animation:`riseIn .3s ease-out ${ci*0.05}s both`}}><div style={{display:"flex",alignItems:"flex-start",gap:12,marginBottom:10}}><span style={{fontSize:28,flexShrink:0,lineHeight:1}}>{c.icon}</span><div style={{flex:1,minWidth:0}}><div style={{fontSize:15,fontWeight:800,color:t.text,lineHeight:1.2,marginBottom:4}}>{c.name}</div><div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}><span style={{background:c.statusColor+"22",border:`1px solid ${c.statusColor}55`,borderRadius:20,padding:"2px 10px",fontSize:10,fontWeight:700,color:c.statusColor,textTransform:"uppercase"}}>{c.status}</span><span style={{fontSize:11,color:t.sub}}>{c.region}</span></div></div><span style={{fontSize:16,color:t.sub,flexShrink:0,marginTop:4}}>→</span></div><div style={{display:"flex",gap:20,marginBottom:10}}><div><div style={{fontSize:10,color:t.sub,marginBottom:1}}>Deaths</div><div style={{fontSize:13,fontWeight:700,color:c.statusColor}}>{c.deaths}</div></div><div><div style={{fontSize:10,color:t.sub,marginBottom:1}}>Displaced</div><div style={{fontSize:13,fontWeight:700,color:t.text}}>{c.displaced}</div></div>{c.warDay&&<div><div style={{fontSize:10,color:t.sub,marginBottom:1}}>War day</div><div style={{fontSize:13,fontWeight:700,color:t.text}}>{c.warDay}</div></div>}</div><div style={{fontSize:12,color:t.sub,lineHeight:1.6,background:t.isDark?"rgba(255,255,255,.03)":"rgba(0,0,0,.03)",borderRadius:8,padding:"9px 11px"}}>{c.summary}</div><div style={{marginTop:10,fontSize:11,color:"#5b8ec8",fontWeight:600}}>Open Deep Dive →</div></button>;})}</div>
  ...
  {[
    ...
    {id:"nuclear",name:"Nuclear Proliferation",icon:"☢️",status:"POST-TREATY",statusColor:"#ef4444",region:"Global",s1l:"New START",s1v:"Expired",s2l:"Iran MoU",s2v:"42 days",summary:"Active vectors, global arsenals, delivery-system triad modernization, and the treaty architecture after New START's Feb 2026 lapse."},
...
function TrendsView({t}){
  ...
  const KPIS=[
    {label:"RU casualties/mo",v:"~38.5k",d:last(HISTORICAL.ruCasualtiesMonthly)-prev(HISTORICAL.ruCasualtiesMonthly),fmt:"abs",color:"#ef4444"},
    {label:"RU gains km²/mo",v:"~11",d:last(HISTORICAL.ruTerritoryKm2)-prev(HISTORICAL.ruTerritoryKm2),fmt:"abs",color:"#eab308"},
    {label:"RU refining offline",v:"~40%",d:last(HISTORICAL.refineryOffline)-prev(HISTORICAL.refineryOffline),fmt:"pt",color:"#22c55e"},
  ...
  return <div>
    ...
    {tab==="timeline"&&<div>
      <Hero t={t} style={{borderLeft:"4px solid #8b5cf6"}}>
        ...
        <div style={{fontSize:12,color:"rgba(255,255,255,.65)",lineHeight:1.6}}>From Operation Epic Fury (Feb 28) through the Islamabad MoU and the 41-day countdown to Aug 16.</div>
      </Hero>
    ...
    {tab==="hormuz"&&<div>
      ...
      <Card t={t}><div style={{padding:"11px 14px"}}><div style={{fontSize:12,color:t.sub,lineHeight:1.7}}>
        <div style={{marginBottom:6}}><span style={{color:t.text,fontWeight:700}}>Partial reopening, uneven traffic</span> — a convoy of five-plus vessels (crude, LPG, LNG, container) transited Jul 5 hugging the Omani coastline, but at least 8 ships U-turned Jul 4. A maritime security authority describes overall traffic as steady but not increasing.</div>
        ...
        <div><span style={{color:t.text,fontWeight:700}}>Leverage logic</span> — Iran is using the funeral's mass turnout and its residual grip on the strait to strengthen its negotiating hand; the US has rejected any Iranian 'measure of control' as a condition. Talks frozen until the funeral ends (~Jul 9-11).</div>
      </div></div></Card>
...
function NuclearSection({t}){
  ...
  const VECTORS=[
    ...
    {flag:"🇮🇷",name:"Iran — Nuclear Clock",status:"41 DAYS",color:"#8b5cf6",text:"Islamabad MoU 60-day window closes ~Aug 16. IAEA inspector access disputed. Enrichment red lines unresolved — Pezeshkian: 'we will never back down from the right to enrich uranium.'"},
